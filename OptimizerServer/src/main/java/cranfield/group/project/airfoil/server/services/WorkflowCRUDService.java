...
public class WorkflowCRUDService extends AbstractCRUDService<Long, Workflow> {
	...
	EntityManagerFactory emf = EntityFactoryProvider.getInstance()
	...
	public Workflow getWorflowWithId(Long id) {
		EntityManager em = emf.createEntityManager();
		em.getTransaction().begin();
		List<Workflow> workflows = em.createNamedQuery("chosenWorkflow")
				.setParameter("id", id).getResultList();

		Workflow chosenWorkflow = workflows.get(0);
		if (chosenWorkflow != null) {
			List<Results> results = em.createNamedQuery("resultsForWorflow")
					.setParameter("workflow", chosenWorkflow).getResultList();
	...
	public void removeWorkflow(Long workflowId) {
		EntityManager em = emf.createEntityManager();
		em.getTransaction().begin();
		Workflow workflow = (Workflow) em.createNamedQuery("chosenWorkflow")
				.setParameter("id", workflowId).getSingleResult();
		if (workflow != null) {
			List<Results> results = em.createNamedQuery("resultsForWorflow")
					.setParameter("workflow", workflow).getResultList();
			if (results != null) {
