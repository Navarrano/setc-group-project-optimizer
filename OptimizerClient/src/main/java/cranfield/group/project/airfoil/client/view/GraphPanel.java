...
"",
 ...
 false, false,
	...
	// public void displayOptimizationRatio(Vector<IterationValuesSet>
	// optimizationResults){
	// removeAll();
	//
	// JFreeChart ratioGraph = ChartFactory.createXYLineChart(
   ...
   private XYDataset createDataset(List<ResultsDTO> workflowResults) {
	...
	// private XYDataset createDataset(Vector<IterationValuesSet>
	// optimizationResults) {
	// XYSeriesCollection dataset = new XYSeriesCollection();
	// XYSeries ratios = new XYSeries("Lift / Drag");
	//
	...
	/*
	 * public static void main( String[ ] args ) { GraphPanel chart = new
	 * GraphPanel(); JFrame window = new JFrame(); window.setContentPane(chart);
